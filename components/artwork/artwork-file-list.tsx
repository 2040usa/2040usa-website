...
export function ArtworkFileList() {
  ...
  return <section className="mt-6" aria-labelledby="stored-artwork-title">
    <h2 id="stored-artwork-title" className="font-display text-2xl uppercase text-text-primary">Draft artwork records</h2>
    <ul className="mt-4 space-y-3">
      {records.map((record) => {
            ...
            : recovery.kind === "remove-invalid"
              ...
              : recovery.kind === "retry-delete"
                ...
                : recovery.kind === "resume" || recovery.kind === "retry"
                  ...
                  : null;
        return <li key={record.id} className="flex min-w-0 flex-col gap-4 border border-border bg-panel p-4 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-text-primary">{record.originalName}</p><p className="mt-1 font-mono text-[0.54rem] uppercase tracking-widest text-text-muted">{record.mimeType} · {formatBytes(record.verifiedSizeBytes ?? record.declaredSizeBytes)} · {record.status}</p>{recoveryExplanation && <p className="mt-2 text-xs leading-5 text-text-muted">{recoveryExplanation}</p>}{record.failureCode && <p className="mt-2 text-xs text-error">Upload status: {record.failureCode.replaceAll("_", " ")}.</p>}</div>
