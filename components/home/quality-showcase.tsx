...
const checks = ["One editable sizing axis per request", "Original-size intent never invents DPI", "Requested changes stay attached to one file"];

export function QualityShowcase() {
  return (
    <section id="quality" className="rounded-control border border-border bg-panel p-5 shadow-[var(--card-shadow)] sm:p-6" aria-labelledby="quality-heading">
      <SectionLabel>Proportional sizing</SectionLabel>
      <h2 id="quality-heading" className="mt-5 font-display text-3xl font-semibold leading-tight text-text-primary sm:text-4xl">Artwork should never be stretched.</h2>
      ...
      <p className="mt-4 text-xs text-text-muted">Counterpart dimensions and original physical size remain pending when reliable file metadata is unavailable.</p>
    </section>
  );
}
