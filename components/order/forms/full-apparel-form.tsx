...
export function FullApparelForm() {
  ...
  return (
    <form noValidate onSubmit={handleSubmit(onSubmit)}>
      ...
      <fieldset aria-describedby="garment-source-error" className="rounded-control border border-border bg-panel p-4 shadow-[var(--card-shadow)] sm:p-5">
        <legend className="px-2 font-display text-xl font-semibold text-text-primary">Garment source</legend>
        <div className="mt-2 grid gap-3 sm:grid-cols-3">{garmentSources.map((value) => <label key={value} className="flex min-h-12 cursor-pointer items-center gap-3 rounded-control border border-border bg-background px-3 text-sm text-text-primary has-checked:border-primary-action has-checked:bg-raised has-focus-visible:outline-2 has-focus-visible:outline-offset-2 has-focus-visible:outline-focus-ring"><input type="radio" value={value} className="size-4 accent-primary-action" {...register("garmentSource")} />{garmentSourceLabels[value]}</label>)}</div>
      ...
      <fieldset aria-describedby="print-locations-error" className="mt-5 rounded-control border border-border bg-panel p-4 sm:p-5">
        <legend className="px-2 font-display text-xl font-semibold text-text-primary">Requested print locations</legend>
        <div className="mt-2 grid gap-3 sm:grid-cols-2">{printLocations.map((value) => <label key={value} className="flex min-h-12 cursor-pointer items-center gap-3 rounded-control border border-border bg-background px-3 text-sm text-text-primary has-checked:border-primary-action has-checked:bg-raised has-focus-visible:outline-2 has-focus-visible:outline-offset-2 has-focus-visible:outline-focus-ring"><input type="checkbox" value={value} className="size-4 accent-primary-action" {...register("printLocations")} />{locationLabels[value]}</label>)}</div>
