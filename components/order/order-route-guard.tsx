...
export function OrderRouteGuard({ step, children }: { step: OrderStepId; children: ReactNode }) {
  ...
  if (needsReadyArtwork && artwork.state === "error") {
    return <div role="alert"><h1 className="font-display text-4xl uppercase text-text-primary">Your artwork could not be checked.</h1><p className="mt-4 text-sm text-error">{artwork.error}</p><ActionButton type="button" className="mt-6" onClick={() => void artwork.reconcile()}>Retry artwork check</ActionButton></div>;
  }
