...
export function OrderSummaryRail({ route, artworkAcknowledged }: { route: OrderRoute; artworkAcknowledged: boolean }) {
  ...
  return (
    <aside className="h-fit min-w-0 rounded-control border border-border bg-panel p-5 lg:sticky lg:top-5" aria-labelledby="draft-summary-title">
      <div className="flex items-center justify-between gap-3"><p id="draft-summary-title" className="font-mono text-[0.6rem] font-bold uppercase tracking-widest text-accent">Current draft</p><StatusBadge tone="neutral">Memory only</StatusBadge></div>
      ...
      <p className="mt-5 border-t border-border pt-4 font-mono text-[0.52rem] uppercase leading-5 tracking-widest text-text-muted">Refreshing the browser clears this prototype draft.</p>
    </aside>
  );
}
