...
export function ArtworkStep() {
  ...
  const continueToConfigure = () => { acknowledgeArtwork(); router.push("/order/configure"); };
  ...
  return (
    <OrderRouteGuard step="artwork">
      {selectedRoute && <div>
        ...
        <StepActions backHref="/order/start" continueLabel="Acknowledge and continue" onContinue={continueToConfigure} />
      </div>}
    </OrderRouteGuard>
  );
}
