    ...
    <OrderRouteGuard step="artwork">
      {selectedRoute && <div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-[0.8fr_1.2fr]">
          ...
          <section className="rounded-control border border-border bg-panel p-5" aria-labelledby="artwork-checklist-title"><h2 id="artwork-checklist-title" className="font-display text-2xl uppercase text-text-primary">Artwork requirements</h2><ul className="mt-5 space-y-3">{ARTWORK_GUIDANCE[selectedRoute].checklist.map((item) => <li key={item} className="flex items-start gap-3 text-sm leading-6 text-text-muted"><Info aria-hidden="true" className="mt-1 shrink-0 text-accent" size={15} />{item}</li>)}</ul>{selectedRoute === "full-apparel" && <p className="mt-4 border-t border-border pt-4 text-xs leading-5 text-text-muted">A file may be a working reference. Uploading it is not production or artwork approval.</p>}</section>
        </div>
        {draftId && <ArtworkUploader draftId={draftId} />}
        <ArtworkFileList />
        <div className={`mt-6 flex items-start gap-4 rounded-control border p-5 ${readiness.ready ? "border-success/60 bg-success/5" : "border-border bg-raised"}`}><CheckCircle2 aria-hidden="true" className={`mt-0.5 shrink-0 ${readiness.ready ? "text-success" : "text-text-muted"}`} size={22} /><div><p className="font-mono text-xs font-bold uppercase tracking-widest text-text-primary">{readiness.ready ? "Artwork ready for this draft" : "Artwork is not ready yet"}</p><p className="mt-2 text-sm leading-6 text-text-muted">Readiness comes from server-verified completed upload records, not the local upload queue.</p></div></div>
