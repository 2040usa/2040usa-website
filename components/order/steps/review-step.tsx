...
export function ReviewStep() {
  ...
  const resetDraft = useOrderDraft((state) => state.resetDraft);
  const summary = configuration ? formatConfigurationSummary(configuration) : [];
  const startOver = () => {
    if (!window.confirm("Clear this in-memory prototype draft and return to the starting point?")) return;
    resetDraft();
  ...
  return (
    <OrderRouteGuard step="review">
      {selectedRoute && configuration && <div>
        ...
        <p className="mt-5 max-w-2xl text-sm leading-6 text-text-muted">Review the information kept in this browser session. No order, price, payment, or production request exists.</p>
        ...
        <section className="mt-4 rounded-control border border-border bg-panel" aria-labelledby="artwork-review-title"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-border p-5"><div><p className="font-mono text-[0.54rem] uppercase tracking-widest text-accent">Artwork step</p><h2 id="artwork-review-title" className="mt-2 font-display text-2xl uppercase text-text-primary">{artworkAcknowledged ? "Guidance acknowledged" : "Not acknowledged"}</h2></div><Link href="/order/artwork" className="font-mono text-[0.6rem] font-bold uppercase tracking-widest text-text-primary underline decoration-accent underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent">Edit artwork step</Link></div><p className="p-5 text-sm text-text-muted">No files are attached to this draft.</p></section>
        ...
        <div className="mt-8 flex justify-start border-t border-border pt-6"><ActionButton type="button" variant="quiet" className="text-error hover:text-text-primary" onClick={startOver}><RotateCcw aria-hidden="true" size={15} /> Start over</ActionButton></div>
      </div>}
    </OrderRouteGuard>
  );
}
