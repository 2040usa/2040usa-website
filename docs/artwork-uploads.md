...
Individual Designs layout decodes intrinsic raster width and height through that same authenticated preview path. Pixels supply aspect ratio only and are never treated as DPI or physical inches. Signed URLs and decoded geometry remain transient client preview state.
