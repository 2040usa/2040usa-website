...
Replacement always reserves a new artwork UUID and private path. Once the replacement is verified, the server rebinds that design's sizes and requested-change instructions from the old UUID to the new UUID. Only then does the existing cleanup path remove the former object and row.
