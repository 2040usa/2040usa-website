...
2. **Artwork & Layout:** `/order/artwork` is the one editable workspace for automatic private uploads, canonical previews, and artwork-linked configuration. Every uploaded artwork UUID owns one compact configuration card. Each Print-Ready Gang Sheet file owns finished width, finished length, and copies; its actual raster file appears independently in the layout preview. Each Individual Designs file owns stable size variants, quantity, and optional requested-change instructions. Its preview panel generates a deterministic 22-inch layout from resolvable geometry, selected layout mode, and transfer spacing.
...
The responsive workspace owns a dedicated layout-preview component boundary. Print-Ready Gang Sheet renders each actual uploaded raster independently with `object-contain` and its matching width, length, and copies. Individual Designs renders Efficient or UUID-grouped placement, automatic 90-degree rotation, calculated length, mode comparison, and honest unresolved-geometry diagnostics. The deterministic heuristic and authority boundary are documented in `docs/gang-sheet-layout.md`.
